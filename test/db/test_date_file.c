...
{
    // 获取并创建数据文件
    char *data_path = get_appdata_path(DATE_FILE);
    if (data_path != NULL)
    {
        printf("数据文件路径: %s\n", data_path);

        if (create_file(data_path) == 0)
        {
            printf("成功创建数据文件\n");
        }
        else
        {
            fprintf(stderr, "创建数据文件失败\n");
        }
    ...
    // 获取并创建设置文件
    char *setting_path = get_appdata_path(SETTING_FILE);
    if (setting_path != NULL)
    {
        printf("设置文件路径: %s\n", setting_path);

        if (create_file(setting_path) == 0)
        {
            printf("成功创建设置文件\n");
        }
        else
        {
            fprintf(stderr, "创建设置文件失败\n");
        }
