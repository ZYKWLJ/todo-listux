#include "../../../include/include.h"
void load_day_year_file(All_Files *all_files, char *path, Date *date)
{
    struct stat buffer;
    char last_file[100];
    // 将整数转换为字符串并与另一个字符串拼接
    sprintf(last_file, "/%d%s", date->year, ".dat");
    const char *filename = strcat(path, last_file);
    LOG_PRINT("filename: %s\n", filename);
    int exists = (stat(filename, &buffer) == 0);
    FILE *file;
    if (!exists)
    {
        file = fopen(filename, "w");
        if (file)
        {
            fclose(file);
        }
    }

    file = fopen(filename, "r");
    if (file)
    {
        // 检查文件是否为空
        if (fseek(file, 0, SEEK_END) == 0)
        {
            long file_size = ftell(file);
            if (file_size == 0)
            {
                fclose(file);
                return;
            }
            fseek(file, 0, SEEK_SET);
        }
        int loaded_year;
        if (fscanf(file, "%d", &loaded_year) != 1)
        {
            LOG_PRINT("Failed to read year from file.\n");
            fclose(file);
            free_day_year_tasks(all_files->total_file_year_date->day_year_task);
            return;
        }
        // 消耗换行符
        fgetc(file);

        if (loaded_year != date->year)
        {
            LOG_PRINT("Year in file does not match requested year.\n");
            fclose(file);
            free_day_year_tasks(all_files->total_file_year_date->day_year_task);
            return;
        }

        for (int m = 0; m < 12; m++)
        {
            // TaskMonth *month_tasks = &year_tasks->months[m];
            TaskMonth *month_tasks = &all_files->total_file_year_date->day_year_task->months[m];
            for (int d = 0; d < month_tasks->num_days; d++)
            {
                TaskDay *day_tasks = &month_tasks->days[d];
                int month_num, day_num, size, capacity;

                // 检查文件结束
                if (feof(file))
                {
                    // 文件正常结束，没有更多数据
                    fclose(file);
                    return;
                }

                // 读取4个整数
                int read_count = fscanf(file, "%d %d %d %d", &month_num, &day_num, &size, &capacity);
                LOG_PRINT("month_num=%d,day_num=%d,size=%d,capacity=%d\n", month_num, day_num, size, capacity);

                if (read_count != 4)
                {
                    // 可能是文件结束或格式错误
                    if (feof(file))
                    {
                        // 文件正常结束
                        fclose(file);
                        return;
                    }
                    else
                    {
                        LOG_PRINT("Failed to read day tasks information from file.\n");
                        fclose(file);
                        free_day_year_tasks(all_files->total_file_year_date->day_year_task);
                        return;
                    }
                }
                // 消耗换行符
                fgetc(file);

                // 检查并扩展容量
                while (size > day_tasks->capacity)
                {
                    expand_capacity(day_tasks);
                }

                day_tasks->size = size;
                day_tasks->capacity = capacity;

                for (int t = 0; t < size; t++)
                {
                    char line[1000000]; // 开的足够大，不怕读取不了！
                    if (fgets(line, sizeof(line), file) == NULL)
                    {
                        LOG_PRINT("Failed to read task line from file.\n");
                        fclose(file);
                        free_day_year_tasks(all_files->total_file_year_date->day_year_task);
                        return;
                    }

                    // 移除末尾的换行符
                    line[strcspn(line, "\n")] = '\0';

                    // 查找最后一个空格来分隔任务内容和完成状态
                    char *last_space = strrchr(line, ' ');
                    if (last_space == NULL)
                    {

                        LOG_PRINT("Invalid task format: %s\n", line);
                        fclose(file);
                        free_day_year_tasks(all_files->total_file_year_date->day_year_task);
                        return;
                    }

                    // 分隔任务内容和完成状态
                    *last_space = '\0';
                    char *task_content = line;
                    char *done_str = last_space + 1;

                    // 分配任务内存
                    size_t content_length = strlen(task_content);
                    if (content_length >= DEFALUT_TASK_CHAR_LENGTH)
                    {
                        char *new_task_str = (char *)realloc(day_tasks->task[t].content,
                                                             (content_length + 1) * sizeof(char));
                        if (new_task_str == NULL)
                        {
                            LOG_PRINT("Memory allocation failed.\n");
                            fclose(file);
                            free_day_year_tasks(all_files->total_file_year_date->day_year_task);
                            return;
                        }
                        day_tasks->task[t].content = new_task_str;
                    }
                    strcpy(day_tasks->task[t].content, task_content);
                    day_tasks->task[t].is_done = atoi(done_str);
                }
            }
        }
        fclose(file);
    }
}