...
void all_execute(Date *date /*指定的日期，不指定默认为当前的日期*/, Date_Type *date_type, Total_File_Year_Date *total_file_year_date, Setting *setting, int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "show") == 0 || strcmp(argv[1], "s") == 0)
    {
        ...
        display(date /*默认的是当前日期，不需要修改*/, date_type, total_file_year_date->day_year_task, setting, argc, argv); // 默认显示当前日期(还是统一吧)
    }
    else if (strcmp(argv[1], "add") == 0 || strcmp(argv[1], "a") == 0)
    {

        add(date /*默认的是当前日期，不需要修改*/, date_type, total_file_year_date->day_year_task, setting, argc, argv);
    }
    else if (strcmp(argv[1], "delete") == 0 || strcmp(argv[1], "d") == 0)
    {

        delete_(date /*默认的是当前日期，不需要修改*/, date_type, total_file_year_date->day_year_task, setting, argc, argv);
    }
    ...
    else if (strcmp(argv[1], "done") == 0)
    {
        ...
        // 至于all还是具体下标，就放在里面判断
        done(date /*默认的是当前日期，不需要修改*/, date_type, total_file_year_date->day_year_task, setting, argc, argv);
    }
    else if (strcmp(argv[1], "undo") == 0) // 撤销完成标记
    {

        undo(date /*默认的是当前日期，不需要修改*/, date_type, total_file_year_date->day_year_task, setting, argc, argv);
    }
